...
{
    private const string TestLocationName = "Test Environment";
    ...
    public async Task<IActionResult> SeedTestData()
    {
        ...
        {
            TempData["Error"] = "Test data already exists. Remove it first if you want to generate it again.";
        ...
        await _locationService.CreateAsync(new CreateLocationDto
        {
            ...
            City    = "Test City",
            Country = "Belgium",
            Notes   = "Automatically generated test data — safe to remove via Developer Tools."
        });
        ...
        {
            LocationId = location.Id, Name = "Server Room Test", Address = "Test Street 1",
            Description = "Test department for network and server equipment."
        });
        await _departmentService.CreateAsync(new CreateDepartmentDto
        {
            LocationId = location.Id, Name = "IT Office Test", Address = "Test Street 2",
            Description = "Test department for office devices."
        });
        await _departmentService.CreateAsync(new CreateDepartmentDto
        {
            LocationId = location.Id, Name = "Security Test", Address = "Test Street 3",
            Description = "Test department for crypto and security equipment — deliberately not yet visited."
        });

        var deptServer = await _context.Departments.FirstAsync(d => d.Name == "Server Room Test" && d.LocationId == location.Id);
        var deptOffice = await _context.Departments.FirstAsync(d => d.Name == "IT Office Test" && d.LocationId == location.Id);
        var deptGuard  = await _context.Departments.FirstAsync(d => d.Name == "Security Test" && d.LocationId == location.Id);
        ...
        await _networkService.CreateAsync(new CreateNetworkDto
        {
            DepartmentId = deptOffice.Id, Name = "Office VLAN", NetworkAddress = "192.168.91.0",
        ...
        await _networkService.CreateAsync(new CreateNetworkDto
        {
            DepartmentId = deptGuard.Id, Name = "Secured VLAN", NetworkAddress = "192.168.92.0",
        ...
        var netOffice = await _context.Networks.FirstAsync(n => n.Name == "Office VLAN" && n.DepartmentId == deptOffice.Id);
        var netGuard  = await _context.Networks.FirstAsync(n => n.Name == "Secured VLAN" && n.DepartmentId == deptGuard.Id);

        // ── 4. Devices — server room ─────────────────────────────────────────
        ...
        // ── 6. Devices — security / crypto ───────────────────────────────────
        var crypto1 = await _deviceService.CreateAsync(new CreateDeviceDto { DepartmentId = deptGuard.Id, NetworkId = netGuard.Id, Name = "CRYPTO-TEST-01", DeviceType = DeviceType.Crypto, Status = DeviceStatus.Active, Notes = "Test device — key expires soon." });
        var crypto2 = await _deviceService.CreateAsync(new CreateDeviceDto { DepartmentId = deptGuard.Id, NetworkId = netGuard.Id, Name = "CRYPTO-TEST-02", DeviceType = DeviceType.Crypto, Status = DeviceStatus.Active, Notes = "Test device — key has already expired." });
        var crypto3 = await _deviceService.CreateAsync(new CreateDeviceDto { DepartmentId = deptGuard.Id, NetworkId = netGuard.Id, Name = "CRYPTO-TEST-03", DeviceType = DeviceType.Crypto, Status = DeviceStatus.Active, Notes = "Test device — key still valid for a long time." });
        ...
        // ── 8. Visits & action items — server room (full history) ───────────
        var visit1Id = await _visitService.CreateVisitAsync(new CreateSiteVisitDto
        {
            DepartmentId = deptServer.Id,
            Summary = "First inspection visit, everything checked.",
            NewItems = new()
            {
                new NewItemInput { Description = "Cable tray loose at the back of the rack", Priority = "High" },
                new NewItemInput { Description = "Switch firmware is outdated",               Priority = "Normal" }
            }
        });
        ...
        var cableTrayItem = openItemsServer.First(i => i.Description.Contains("Cable tray"));

        var visit2Id = await _visitService.CreateVisitAsync(new CreateSiteVisitDto
        {
            DepartmentId = deptServer.Id,
            Summary = "Follow-up visit.",
            ResolvedItems = new() { new ResolvedItemInput { ActionItemId = cableTrayItem.Id, ResolutionNotes = "Fastened with new cable clips." } },
            NewItems = new() { new NewItemInput { Description = "UPS battery needs to be tested", Priority = "Low" } }
        });
        ...
        // ── 9. Visit — office (only 1, for variety) ──────────────────────────
        var visit3Id = await _visitService.CreateVisitAsync(new CreateSiteVisitDto
        {
            DepartmentId = deptOffice.Id,
            Summary = "Office checked, minor remark.",
            NewItems = new() { new NewItemInput { Description = "Printer makes a strange noise on startup", Priority = "Normal" } }
        });
        await SetVisitDateAsync(visit3Id, DateTime.UtcNow.AddDays(-10));

        // Security Test deliberately gets no visit — to show the empty state.
        ...
        {
            DepartmentId = deptServer.Id,
            Notes = "Monthly server room check.",
            Items = new()
            {
                ...
                new CreateInventoryCheckItemDto { DeviceId = upsId, IsPresent = false, Remark = "Not found during check — to follow up with the department." },
                new CreateInventoryCheckItemDto { DeviceId = nasId, IsPresent = true, PhotoData = testPhoto, PhotoContentType = "image/png", PhotoFileName = "test-photo.png" }
            }
        });
        ...
        var check2Id = await _checkService.CreateAsync(new CreateInventoryCheckDto
        {
            ...
            Items = new()
            {
                ...
                new CreateInventoryCheckItemDto { DeviceId = prtId, IsPresent = true, Remark = "Was switched off on arrival, turned back on." },
                new CreateInventoryCheckItemDto { DeviceId = telId, IsPresent = true }
            }
        });
        await SetCheckDateAsync(check2Id, DateTime.UtcNow.AddDays(-1));

        // Security Test deliberately gets no check — to show the empty state.

        TempData["Success"] = "Test data created under location 'Test Environment'.";
    ...
    public async Task<IActionResult> DeleteTestData()
    {
        ...
        if (location == null)
        {
            TempData["Error"] = "No test data found.";
            return RedirectToAction(nameof(Index));
        }
        ...
        TempData["Success"] = "Test data removed.";
        return RedirectToAction(nameof(Index));
    }
